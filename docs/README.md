...
| [Authentication](authentication.md) | Inbound client validation, per-worker backend API keys, embeddings endpoint auth, health probe exemptions, security best practices |
| [Load Balancing](load-balancing.md) | Policy overview with defaults, use-case recommendations, multi-turn routing, per-policy details, decision tree |
| [Semantic Routing](semantic-routing.md) | Cluster routing by prompt content with embeddings, embeddings API key support |
| [Caching](caching.md) | Exact-match and semantic response cache pipeline, Redis backend, embeddings API key support |
